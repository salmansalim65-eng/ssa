...
describe("computePropertyRow — reference formulas", () => {
  ...
  it("Net Rent (monthly) = Monthly Rent − Commission − HH expenses (no commission/expenses here)", () =>
    expect(r.netRent).toBe(2505));
  it("Perc% = Net Rent × 12 / Current Value × 100", () => expect(r.perc).toBe(5.47));
  it("% Month = Perc% / 12", () => expect(r.percMonth).toBe(0.46));
