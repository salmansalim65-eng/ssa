...
/**
 * Converts an invoice amount held in its document currency to the selected
 * report currency using the invoice's OWN booking rate for the document→base
 * leg (base = documentAmount × bookingRate — the exact base posted at invoice
 * time) and the report-date `factor` from resolveReportCurrency for base→selected.
 */
export function convertAtBookingRate(documentAmount: number, bookingRate: number, factor: number): number {
  return documentAmount * (bookingRate || 1) * factor;
}
