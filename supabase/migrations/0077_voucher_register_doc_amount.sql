-- The voucher register (and the dashboard's Recent transactions table) showed a
-- single base-currency amount. To display each voucher in its own currency, add
-- doc_amount — the document-currency total (sum of line debit amounts) alongside
-- the existing base-currency amount and the already-exposed currency_id.
-- Additive (appended last), so existing consumers of `amount` are unaffected.
...
select
  ...
  coalesce(rv.voucher_no, pv.voucher_no, ppv.voucher_no, prv.voucher_no, crv.voucher_no,
           ...
           uri.voucher_no, pri.voucher_no, asv.voucher_no) as voucher_no,
  je.currency_id,
  je.status,
  je.narration,
  je.created_by,
