-- The dashboard "Balances UAE"/"Balances PK" cards attribute each posting to a
-- country via its cost centre. Party accounts (suppliers, customers) booked
-- without a cost centre — e.g. a manually-entered opening balance — therefore
-- fall out of both cards even though the account itself is clearly UAE or PK.
--
-- Expose the account's own ISO country (chart_of_accounts.country, captured in
-- the account Details) so the dashboard can attribute a line by
-- coalesce(cost_center_country, account_country). Additive column at the end,
