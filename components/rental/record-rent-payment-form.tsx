...
import { recordUaeRentPayment } from "@/features/rental/uae-rent-payments/actions";
import {
  recordPaymentSchema,
  type RecordPaymentFormValues,
  type RecordPaymentInput,
} from "@/features/rental/uae-rent-invoices/schemas";

function today() {
  return new Date().toISOString().slice(0, 10);
}

export function RecordRentPaymentForm({ invoiceId, accounts }: { invoiceId: string; accounts: AccountOption[] }) {
  ...
  function onSubmit(values: RecordPaymentInput) {
    ...
    startTransition(async () => {
      const result = await recordUaeRentPayment(invoiceId, values);
      if (result?.error) {
